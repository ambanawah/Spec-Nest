...
export default function LaptopsCatalogPage() {
    ...
    useEffect(() => {
        setLoading(true);
        const params = new URLSearchParams({ category: 'laptops', sortBy, sortOrder, currency });
            ...
            <main className="pt-24 pb-32 min-h-screen px-4 md:px-8 max-w-[1600px] mx-auto flex flex-col md:flex-row gap-8">
                ...
                <section className="flex-1 space-y-8">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h1 className="font-headline text-3xl font-extrabold tracking-tighter text-on-surface">Laptops</h1>
                    ...
                    {products.length === 0 && !loading && (
                        <p className="text-center text-on-surface-variant py-24">No laptops found.</p>
                    )}
                </section>
            </main>
            <ComparisonTray />
